...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (thumbnailsContainer) {
        thumbnailsContainer.innerHTML = productImages
            .map(
                (src, i) => `
            <button onclick="changeMainImage(${i})" class="w-16 h-16 rounded-lg border-2 overflow-hidden shrink-0 product-thumb transition-all ${i === 0
                        ? "border-primary"
                        : "border-transparent opacity-70 hover:opacity-100 hover:border-gray-300"
                    }">
        ${src.endsWith(".mp4") ? `<video muted
        playsinline
        preload="metadata" src="${src}" class="w-full h-full object-cover"><source src="${src}" type="video/mp4"></video>` : `<img src="${src}" class="w-full h-full object-cover" />`}
    ...
    window.changeMainImage = (index) => {
        ...
        updateMediaElement("mainProductImage", productImages[index], !isLightboxOpen);
    ...
    window.openLightbox = () => {
        ...
        if (lightboxThumbnailsContainer) {
            lightboxThumbnailsContainer.innerHTML = productImages
                .map(
                    (src, i) => `
                <button onclick="updateLightboxImage(event, ${i})" class="w-16 h-16 md:w-20 md:h-20 shrink-0 border-2 rounded-lg overflow-hidden transition-all ${i === currentImageIndex
                            ? "border-primary"
                            : "border-transparent opacity-50 hover:opacity-100"
                        }">
                   ${src.endsWith(".mp4") ? `<video muted playsinline preload="metadata" src="${src}" class="w-full h-full object-cover"><source src="${src}" type="video/mp4"></video>` : `<img src="${src}" class="w-full h-full object-cover" />`}
                </button>
            `
                )
                .join("");
